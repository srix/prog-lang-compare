...
def write_to_file( ai_answer, prog_lang, concept, subconcept, llm_name='gpt-3.5-turbo'):
    ...
    # [UPDATED PATH] Point to output directory relative to src/
    directory = f'../output/content-autogen/{llm_name}/{prog_lang_name}/'
...
def build_content(prog_langs,lang_concepts,  llm_name='gpt-3.5-turbo'):
    ...
    for prog_lang in prog_langs:
        prog_lang_name = helper.get_safename(prog_lang)
        # [UPDATED PATH]
        folder_path = f'../output/content-autogen//{llm_name}/{prog_lang_name}/'
        for concept, value in lang_concepts.items():
            if isinstance(value, dict):
                for subconcept, prompt in value.items():
                    file_name = f'{helper.get_safename(concept)}_{helper.get_safename(subconcept)}.md'
         ...
         # Open the file with write permission
        # [UPDATED PATH]
        merge_file_path = os.path.join( f'../output/content-autogen//{llm_name}/', f'{prog_lang_name}.json')
...
# [UPDATED PATHS]
PROG_LANG_CONCEPTS_PATH = '../config/prog_lang_concepts.yaml'
PROG_LANGS_PATH = '../config/prog_langs.yaml'
